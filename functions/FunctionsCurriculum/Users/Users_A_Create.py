...
def users_a_create(data):
    ...
    user = auth.create_user(email=user.get("Email__c"))

    # Save the user UID to the Salesforce record
    sf.sf.Employee__c.update(data.get("Id"), {"Firebase_UID__c": user.uid})

    # Link SF ID to Firebase UID
    auth.set_custom_user_claims(user.uid, {"sfid": data.get("Id")})

    # Generate password reset link
    reset_link = auth.generate_password_reset_link(user.get("Email__c"))

    # Send email to the user
    send_email_user(user.get("Email__c"), "Account Created for ILPlatform",
        f"""
        ...
        <p>Your account has been created using the email address: <strong>{user.get("Email__c")}</strong>.</p>
