...
def send_email_replacement_permanent(email, class_name, date):
...
def send_email_replacement_permanent_admin(email, class_name, date):
    # Setup template parameters
    params = {
        'email': email,
        'class_name': class_name,
    ...
    # Setup email parameters
    subject = "Nouveau remplacement PERMANENT"
...
def html_replacement(params):
    ...
    <p>
        Ta demande de remplacement <b>permanente</b> a bien été reçue pour :
    </p>
    <ul>
        <li><b>Cours :</b> {params.get('class_name')}</li>
        <li><b>À partir du :</b> {params.get('date')} (inclus)</li>
    </ul>
    <p>
        Si cette demande est de dernière minute (moins d'une semaine), merci de nous contacter également via WhatsApp au plus vite.
    </p>
    <p>
...
def html_replacement_admin(params):
    ...
    <p>
        Une demande de remplacement <b>permanente</b> a été reçue pour :
    </p>
    <ul>
